Ext.define('sisbotica_paulino.view.puntoventa.AccionesContenedorMain', {
    ...
    onClickIngresarPago:function(btn){
      try {
        if(Ext.ComponentQuery.query('#dgvDetalleCaja')[0].getStore().getCount()==0){
           sisbotica_paulino.util.Util.showToast("TIENE QUE INGRESAR EL DETALLE DE LA VENTA"); return false;
        }
    ...
    onClickNuevoClientePDV:function(btn){
      sisbotica_paulino.util.Util.crearWindowOpenMantenimiento('Agregar Cliente',650,600,'cboCliente','wFormClienteListado',0);
      
    },
    onClickEditarClientePDV:function(btn){
      sisbotica_paulino.util.Util.crearWindowOpenMantenimiento('Editar Cliente',650,600,'cboCliente','wFormClienteListado',0);
      Ext.ComponentQuery.query('#wFormClienteListado')[0].loadRecord(btn.record);
    },
    onClickApeturarCaja:function(btn){
      Ext.create('sisbotica_paulino.view.puntoventa.AperturaCaja');
    },
