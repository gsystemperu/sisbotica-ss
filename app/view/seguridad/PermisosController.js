Ext.define('sisbotica_paulino.view.seguridad.PermisosController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.seguridad-permisos',
    init: function () {
        r = this.lookupReference('treeprog');
        root = r.getRootNode();
        ca = Ext.create('sisbotica_paulino.store.tree.ControlAlmacen');
        cv = Ext.create('sisbotica_paulino.store.tree.ControlVentas');
        m = Ext.create('sisbotica_paulino.store.tree.Mantenimiento');
        cu = Ext.create('sisbotica_paulino.store.tree.ControlUsuarios');
        this.setCargarMenu(root, ca, cv, m, cu);
    },
    setCargarMenu(t, ca, cv, ma, cu) {
        menu = {
            text: sisbotica_paulino.Global.empresa,
            expanded: true,
            children: []
        };
        nodo = [];
    ...
    onClickActualizarPermisosAlPerfil: function (b) {
        ...
        if (this.lookupReference('cboperfil').getValue() != null) {
            ...
            sisbotica_paulino.util.Util.ajax(sisbotica_paulino.util.Rutas.actualizarPermisos, data, null);
        }
        else
    ...
    onChangeBuscarPermisosPerfil: function (obj, newValue, oldValue, eOpts) {
        me = this;
        Ext.Ajax.request({
            url: sisbotica_paulino.util.Rutas.permisosDelPerfil,
            params: {
                idperfil: newValue
            },
            success: function (conn, response, options, eOpts) {
                r = sisbotica_paulino.util.Util.decodeJSON(conn.responseText);
                p = [];
                  try {
                    if(r.data.length){
    ...
    onClickAgregarPerfil:function(b){
        me = this;
        Ext.Ajax.request({
            url: sisbotica_paulino.util.Rutas.perfilActualizar,
