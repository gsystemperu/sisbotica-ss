...
Ext.define('sisbotica_paulino.view.almacen.AccionesProducto', {
        ...
        try {
            ...
            if(record.get('fotoprod')=='P-00.jpg'){
              ...
              Ext.ComponentQuery.query('#imgprod')[0].setSrc(
                sisbotica_paulino.util.Rutas.srcimagenes +  'P-00.jpg?_='+(new Date().getTime())
              )
            }else{
              t = new Date().getTime();
              Ext.ComponentQuery.query('#imagenguardar')[0].setValue(1);
              Ext.ComponentQuery.query('#imgprod')[0].setSrc(
                sisbotica_paulino.util.Rutas.srcimagenes + record.get('fotoprod')+'?_='+t.toString()
              )
            }
            
            Ext.Ajax.request({
                url :sisbotica_paulino.util.Rutas.productoBuscarProveedores,
    ...
    onClickIngresarUbicacionProducto:function(btn){
        Ext.create('sisbotica_paulino.view.almacen.ProductoUbicacion', {codigo : btn.getWidgetRecord().get('id')});
    },
    ...
    onClickEliminarProductoSerie:function(btn){
      ...
      if(r){
        Ext.Ajax.request({
            url :sisbotica_paulino.util.Rutas.productoEliminarSerie,
            params:{
              id : r.get('id')
            },
            success:function(response){
    ...
    onClickCopiar:function(b){
      ...
      t= o.getValue();
      o.setValue(t + ' (copia)');
      sisbotica_paulino.util.Util.focusControl('nombre'); 
    },
    onClickEditarProductoSerie:function(b){
      r = b.getWidgetRecord();
      Ext.create('sisbotica_paulino.view.almacen.ProductoExistenciasEditar');
