...
Ext.define('sisbotica_paulino.view.almacen.AccionesAlmacen', {
    ...
    onClickEliminarAlmacen:function(btn){
        me = this;
        Ext.Ajax.request({
                url: sisbotica_paulino.util.Rutas.almacenEliminar, 
    ...
    onClickNuevoSeccion:function(btn)
    {
     ...
     onClickEliminarSeccion:function(btn){
        me = this;
        Ext.Ajax.request({
                url: sisbotica_paulino.util.Rutas.seccionAlmacenEliminar, 
                params: { id          : btn.getWidgetRecord().get('id')},
                success: function(response){
                    var _error = Ext.JSON.decode(response.responseText);
                    if(_error.error!=0){
