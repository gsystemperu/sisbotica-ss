Ext.define('sisbotica_paulino.view.compras.AccionesContenedorOrdenCompra', {
    ...
    onClickConfirmarOrdenCompra:function(){
      ...
      if (_rec) {
          Ext.Ajax.request({
              url: sisbotica_paulino.util.Rutas.ordenCompraConfirmar,
              params: {
                  id: _rec.get('id')
              },
    ...
    onClickImprimirPdfOrdenCompra: function (btn) {
        r =  Ext.ComponentQuery.query('#gridOrdenesCompra')[0].getSelectionModel().getSelection()[0];
        if (r) {
            objrpt = window.open( sisbotica_paulino.util.Rutas.ordenCompraPdf+ '?id='+ r.get('id') , "", "width=700,height=900");
